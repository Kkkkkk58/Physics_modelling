...
class App(customtkinter.CTk):
    WIDTH = 1080
    HEIGHT = 720
    REQUESTS = {Values.NORMALIZATION_COEFFICIENT: "Нормализующий коэффициент A",
                Values.COERCIVE_FORCE: "Коэрцитивная силу Hc",
                Values.SIGMA: "Cреднеквадратичное отклонение сигма",
    ...
    def __init__(self):
        ...
        self.atherton_button = customtkinter.CTkButton(master=self.frame_right, height=40, fg_color=None,
                                                       text="Модель Атертона",
        ...
        self.preisach_button = customtkinter.CTkButton(master=self.frame_right, height=40, fg_color=None,
                                                       text="Модель Прейзаха",
    ...
    def init_preisach_left_frame(self):
        # Input A_preisach
        self.label_normalize_preisach = customtkinter.CTkLabel(master=self.frame_left,
                                                               text=App.REQUESTS[Values.NORMALIZATION_COEFFICIENT],
        ...
        self.label_coercive_preisach = customtkinter.CTkLabel(master=self.frame_left,
                                                              text=App.REQUESTS[Values.COERCIVE_FORCE],
                                                              text_font=("Roboto Medium", -16))
        self.label_coercive_preisach.grid(row=6, column=0, pady=0, padx=10, sticky="sn")
        self.entry_coercive_preisach = customtkinter.CTkEntry(master=self.frame_left, width=80,
                                                              placeholder_text="20 А/м")
    ...
    def init_atherton_left_frame(self):
        ...
        for key in App.REQUESTS.keys():
            ...
            curr_row += 1
            self.atherton_entries.append(
                customtkinter.CTkEntry(master=self.frame_left, width=80, placeholder_text=placeholder_text[count - 3]))
    ...
    def start_plotting_preisach(self):
        self.plotter = PlotPreisach(self.frame_mid, *self.parameters_pack)
    ...
    def confirm_event_atherton(self):
        ...
        if not self.is_isotropic:  # @TODO Дорогой дневник мне не описать словами ту боль и унижение что я испытал увидев этот блядский флаг
    ...
    def start_plotting_atherton(self):
        self.plotter = PlotJilesAtherton(self.frame_mid, *self.parameters_pack)
        self.plotter.display()

    def switch_anisotropic(self):
        if not self.is_isotropic.get():
